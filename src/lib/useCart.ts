...
export type CartProduct = Product & { 
  quantity: number;
  selectedVariant?: { _key?: string; name?: string; priceModifier?: number };
};
      ...
      (set, get) => ({
        ...
        addToCart: (product) =>
          set((state) => {
            ...
            const shippingCost = newCartItems.length === 0 ? 0 : state.shippingCost;
      ...
      { 
        ...
        merge: (persistedState, currentState) => {
          if (persistedState && typeof persistedState === 'object' && 'cartItems' in persistedState) {
            ...
            const shippingCost = toSafeNumber((persistedState as any).shippingCost);
            const discountAmount = toSafeNumber((persistedState as any).discountAmount);
            return {
              ...currentState,
              cartItems,
              totalPrice: subtotal,
              ...
              discountAmount,
              finalTotal: (persistedState as any).finalTotal || roundToTwoDecimals(subtotal + shippingCost - discountAmount),
            };
          }
          return currentState;
        }
