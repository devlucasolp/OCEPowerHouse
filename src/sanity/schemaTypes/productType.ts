export default {
  ...
  fields: [
    ...
    // CAMPO DE FRETE FIXO REMOVIDO - AGORA USA MELHOR ENVIO
    { 
      name: 'category', 
      title: 'Categoria', 
      type: 'reference',
      ...
      of: [
        {
          ...
          fields: [
            ...
            {
              name: 'stockQuantity',
              title: 'Quantidade em Estoque da Variante',
              type: 'number',
              description: 'Se definido, sobrepõe a quantidade geral (estoque específico da variação)',
              validation: (Rule: any) => Rule.min(0).error('Quantidade em estoque não pode ser negativa')
            }
          ],
          preview: {
