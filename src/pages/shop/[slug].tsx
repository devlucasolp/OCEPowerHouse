        ...
        <div className="flex flex-col gap-6">
          ...
          {product.variants && product.variants.length > 0 && (
            <div className="flex flex-col gap-2">
              <span className="font-medium text-blue-900">Escolha uma opção:</span>
              <div className="flex flex-wrap gap-2">
                {product.variants.map((variant, idx) => (
                  <button
                    key={variant.name}
                    className={`px-4 py-2 rounded-lg border font-semibold transition-all duration-200 ${selectedVariant === idx ? 'bg-yellow-400 text-blue-900 border-yellow-500 shadow' : 'bg-white text-blue-900 border-yellow-200 hover:bg-yellow-100'}`}
